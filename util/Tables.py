...
async def submit_table(ctx: commands.Context, lb: LeaderboardConfig, table: TableBasic, bypass_confirmation = False) -> Table | None:
    ...
    e.add_field(name="View on website", value=(f"{lb.website_credentials.url}/{f'{
        lb.website_credentials.game_url if lb.website_credentials.game_url else lb.website_credentials.game
        }/' if lb.website_credentials.game else ""}TableDetails/{sent_table.id}"), inline=False)
